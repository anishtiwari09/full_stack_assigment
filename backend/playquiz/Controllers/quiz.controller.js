...
const updateQuizDetails = async (req, res) => {
    try {
        ...
        let addValue = 5;
        let level = 1
        let currentQuestion=data[0]?.currentQuestion||1
        let currentLevel=data[0]?.currentLevel||0
        ...
        data = await Quiz.findOneAndUpdate({ test_name: req?.query?.quizId, "user_ids.userId": req?.userData?.id }, {
            $inc: {
                 "user_ids.$.score": addValue, "user_ids.$.currentLevel": level, "user_ids.$.attempt": 1
            },
            $push: { "user_ids.$.data_varient": [data[0].score +addValue, data[0].attempt + 1] },
            $set:{
                "user_ids.$.quizCompleted": quizCompleted,
                "user_ids.$.currentQuestion": currentQuestion
            }
        })
